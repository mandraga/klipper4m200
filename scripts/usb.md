...
Until the UMS mode works, we can use scripts to break the image into 16MB files and load them one by one from a memory stick to the EMMC1 using the serial port to do the reassembling.
...
Login and then dump the firmware on a USB drive
